...
def preprocess_op():
    ...
    return dsl.ContainerOp(
        ...
        image='japisuru/kubeflow-pipelines-preprocess-data:latest',
        arguments=[],
        file_outputs={
...
def train_op(x_train, y_train):
    ...
    return dsl.ContainerOp(
        name='Train Model',
        image='japisuru/kubeflow-pipelines-train:latest',
        arguments=[
            '--x_train', x_train,
            '--y_train', y_train
        ],
...
def test_op(x_test, y_test, model):
    ...
    return dsl.ContainerOp(
        name='Test Model',
        image='japisuru/kubeflow-pipelines-test:latest',
        arguments=[
            '--x_test', x_test,
            '--y_test', y_test,
            '--model', model
...
def deploy_model_op(model):
    ...
    return dsl.ContainerOp(
        name='Deploy Model',
        image='japisuru/kubeflow-pipelines-deploy-model:latest',
        arguments=[
            '--model', model
        ]
    )
